...
def recv_data(sock, save_location, zipFlag):
    ...
    if buffer:
        ...
        if (os.path.exists(save_path_download)):
            ...
            sock.send(GetFileMd5(save_path_download).encode("utf-8")) ###发送断点文件的MD5码,用于校验MD5是否更改(32Byte)
            recv_client_data = sock.recv(1)
            if(int(recv_client_data)==1):## 收到1表示无误,0表示有误
                fp = open(save_path_download, "ab")  # 使用追加命令
                recvd_size = int.from_bytes(recvd_size_befor, byteorder="big")
            else:
                os.remove(save_path_download)
                print("Unable to resume because of the file changes")
                exist_flag=3 ##3表示文件的更改导致的重新断点续传
        if (not os.path.exists(save_path_download)):
            if(exist_flag!=3):
                exist_flag = "0"
                sock.send(exist_flag.encode('utf-8'))
            if zipFlag == 1:
