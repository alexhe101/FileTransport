...
# 发送方
# 使用示例：
# python3 client.py abcd 123.123.123.123 1234
...
def GetFileMd5_break(filepath,break_point_data):
    # 获取文件的md5
    myhash = hashlib.md5()
    f = open(filepath, "rb")
    while break_point_data:
        if(break_point_data>=8096):
            b = f.read(8096)
            break_point_data-=8096
        else:
            b=f.read(break_point_data)
            break_point_data=0
        myhash.update(b)
    f.close()
    return myhash.hexdigest()
...
def transportFile(sock, f, fInfo, zipFlag):
    ...
    if (int(recv_data) == 1):  ###追加
        recv_data_filesize = int.from_bytes(sock.recv(10), byteorder="big")  ### 获取
        recv_md5 = sock.recv(32).decode('utf-8')
        if(recv_md5==GetFileMd5_break(absolute,recv_data_filesize)):
            send_data="1"
            sock.send(send_data.encode("utf-8"))
            print(f"断点续传{absolute}")
            fp = open(absolute, 'rb')
            fp.read(recv_data_filesize)
        else:
            print("现在的MD5",GetFileMd5_break(absolute,recv_data_filesize))
            print("接收的MD5",recv_md5)
            print("该文件曾经被改变,无法断点续传")
            recv_data="0".encode('utf-8')
            send_data='0'
            sock.send(send_data.encode('utf-8'))
